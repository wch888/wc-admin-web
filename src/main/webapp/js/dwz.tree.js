 	...
 	$.extend($.fn, {
		...
		_check:function(op) {
			...
			ckbox.append("<input type='checkbox' style='display:none;' " + attrs + "/>").click(function(){
				...
				ckbox.removeClass(rClass).removeClass(!cked ? "checked" : "").addClass(aClass);
				$("input", ckbox).attr("checked", !cked);
				$(">ul", node).find("li").each(function(){
					var box = $("div.ckbox", this);
					box.removeClass(rClass).removeClass(!cked ? "checked" : "").addClass(aClass)
		...
		_checkParent:function(){
			...
			//var aClass = (ckboxed == ckbox ? "checked" : (ckboxed != 0 ? "checked" : "unchecked"));
			////父亲
			//var rClass = (ckboxed == ckbox ? "checked" : (ckboxed != 0 ? "checked" : "checked"));
			////console.log(parent.html());
			//$(">div>.ckbox", parent).removeClass("unchecked").removeClass("checked").removeClass(rClass).addClass(aClass);
			////$(this).find(".ckbox").removeClass("unchecked").removeClass("checked").removeClass(rClass).addClass(aClass);
